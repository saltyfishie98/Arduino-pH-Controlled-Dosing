...
void MotorControl::begin(){
	...
	if(motorptr->SpeedPin != -1){pinMode(motorptr->SpeedPin,OUTPUT);}
	motorptr->ID = id;
	id++;
}
...
/*!
* @brief Set the "Period" in millisecond\s of a dose, i.e. how long a single dose last 
...
void MotorControl::dose(int quantity){
	for(int i=0;i<quantity;i++){
		...
		off();
		if(i!=quantity)delay(50);
	}
}

void MotorControl::verbose(unsigned int baud){
	motorptr->Verbose = 1;
	Serial.begin(baud);
}
